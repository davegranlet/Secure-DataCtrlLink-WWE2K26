...
constexpr std::uint32_t inverse_odd(std::uint32_t value) {
    ...
    for (unsigned i = 0; i < 5; ++i) inverse *= 2u - value * inverse;
    return inverse;
}
...
void write_u32(std::uint8_t* data, std::uint32_t value) {
    for (unsigned i = 0; i < 4; ++i) data[i] = static_cast<std::uint8_t>(value >> (i * 8));
}
std::uint32_t undo_xor_right(std::uint32_t value, unsigned shift) {
    std::uint32_t result = value;
    for (unsigned width = shift; width < 32; width += shift) result ^= value >> width;
    return result;
}
std::uint32_t key_from_first_mask(std::uint32_t mask) {
    auto mixed = undo_xor_right(mask, 13) * kInverseC2;
    mixed = undo_xor_right(mixed, 16); mixed = rotate_right(mixed, 7); mixed *= kInverse85;
...
std::array<std::uint8_t, 84> decode_header(const std::array<std::uint8_t, 84>& input,
    ...
    std::uint32_t key = initial_key, counter_a = 0, first_state = 0, state_b = 0, previous_first = 0;
    std::size_t cursor = 0;
    for (unsigned block = 0; block < 10; ++block) {
        key ^= state_b;
        if (first_state != 0) {
            state_b += 0xC2B2AE35u; key ^= first_state;
            key = rotate_left(key, 11) + 0x165667B1u;
        }
        auto mixed = (key ^ counter_a ^ 0xA3C59AC3u) * 0x85EBCA6Bu;
        mixed = rotate_left(mixed, 7); mixed ^= mixed >> 16;
        auto mask = mixed * 0xC2B2AE35u; mask ^= mask >> 13;
        const auto cipher_first = read_u32(input.data() + cursor);
        write_u32(output.data() + cursor, cipher_first ^ mask); cursor += 4;
        first_state = cipher_first ^ previous_first;
        if (key != 0) {
            first_state ^= key; first_state = rotate_left(first_state, 7) - 0x7A143595u;
            previous_first += 0x9E3779B9u;
        }
        mixed = ((counter_a - 0x61C88647u) ^ first_state ^ 0x1B873593u);
        counter_a += 0x3C6EF372u; mixed *= 0x85EBCA6Bu;
        mixed = rotate_left(mixed, 7); mixed ^= mixed >> 16;
        mask = mixed * 0xC2B2AE35u; mask ^= mask >> 13;
        const auto cipher_second = read_u32(input.data() + cursor); key = cipher_second;
        write_u32(output.data() + cursor, cipher_second ^ mask); cursor += 4;
    }
    for (unsigned tail = 0; tail < 4; ++tail) {
        const std::uint32_t absolute = 80 + tail;
        const auto index_mix = absolute * 0x27D4EB2Du + 0x7F4A7C15u;
        state_b ^= key; auto rotation = (first_state ^ key) & 0xfu;
        if (tail == 0) rotation = 11;
        first_state ^= state_b; first_state = rotate_left(first_state, rotation);
        first_state -= tail == 0 ? 0xE9A9984Fu : 0x7A143595u;
        auto mixed = first_state ^ index_mix; mixed ^= mixed >> 15;
        const auto mask = mixed * 0x85EBCA6Bu; const auto cipher = input[80 + tail]; key = cipher;
        output[80 + tail] = static_cast<std::uint8_t>(cipher ^ (mask & 0xff) ^ ((mask >> 13) & 0xff));
...
bool header_valid(const std::array<std::uint32_t, 21>& words, std::uint64_t archive_size) {
    if (words[0] > 2'000'000 || words[1] > 1'000'000) return false;
    const std::array<unsigned, 5> size_indexes{3, 6, 9, 12, 15};
    const std::array<unsigned, 5> offset_indexes{5, 8, 11, 14, 17};
    if (words[3] != std::uint64_t(words[1]) * 12 || words[6] != std::uint64_t(words[0]) * 12) return false;
    for (unsigned i = 0; i < 4; ++i)
        if (std::uint64_t(words[offset_indexes[i]]) + words[size_indexes[i]] != words[offset_indexes[i + 1]]) return false;
...
bool validate_cak(const std::filesystem::path& path, CakInfo& info, std::string& error) {
    // SECURITY BOUNDARY: this function receives untrusted mod archives. Keep
    // every count capped and check arithmetic before indexing or allocating.
    info = {}; error.clear(); std::error_code ec;
    const auto size = std::filesystem::file_size(path, ec);
    if (ec || size < 92 || size > 8ull * 1024 * 1024 * 1024) { error = "invalid archive size"; return false; }
    std::ifstream input(path, std::ios::binary); std::array<std::uint8_t, 92> prefix{};
    if (!input.read(reinterpret_cast<char*>(prefix.data()), prefix.size())) { error = "could not read CAK header"; return false; }
    constexpr std::array<std::uint8_t, 8> expected{'F','D','I','R',0x09,0x09,0x00,0x81};
    if (!std::equal(expected.begin(), expected.end(), prefix.begin())) { error = "unexpected CAK magic/version"; return false; }
    std::array<std::uint8_t, 84> encrypted{}; std::memcpy(encrypted.data(), prefix.data() + 8, encrypted.size());
    ...
    for (std::uint32_t count = 0; count <= 2'000'000; ++count) {
        ...
        for (unsigned i = 0; i < words.size(); ++i) words[i] = read_u32(decoded.data() + i * 4);
        if (words[0] == count && header_valid(words, size)) {
            info = {words[0], words[1], key}; return true;
        }
    }
    error = "encrypted CAK catalog header failed structural validation"; return false;
}
