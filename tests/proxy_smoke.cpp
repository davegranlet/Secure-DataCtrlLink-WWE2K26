...
int wmain(int argc, wchar_t** argv) {
    if (argc != 2) { std::cerr << "usage: proxy_smoke <dinput8.dll>\n"; return 2; }
    HMODULE proxy = LoadLibraryW(argv[1]);
    if (!proxy) { std::cerr << "LoadLibrary failed: " << GetLastError() << '\n'; return 1; }
    constexpr std::array<const char*, 6> names = {
        "DirectInput8Create", "DllCanUnloadNow", "DllGetClassObject", "DllRegisterServer",
        "DllUnregisterServer", "GetdfDIJoystick"
    };
    for (const char* name : names) {
        if (!GetProcAddress(proxy, name)) { std::cerr << "missing export: " << name << '\n'; return 1; }
    }
    ...
    if (!get_joystick()) { std::cerr << "system forwarding returned null joystick format\n"; return 1; }
